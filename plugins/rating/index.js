(function () {
  ...
  function getConfig() {
    return {
      ...
      rating_url: Lampa.Storage.get(
        "rating_rating_url",
        DEFAULT_CONFIG.rating_url
      ),
      api_key: Lampa.Storage.get("rating_api_key", DEFAULT_CONFIG.api_key),
      cache_time:
        Lampa.Storage.get("rating_cache_time", DEFAULT_CONFIG.cache_time) *
        60 *
        60 *
        1000,
    };
  ...
  function initSettings() {
    ...
    Lampa.SettingsApi.addParam({
      ...
      onChange: function (value) {
        Lampa.Storage.set(
          "rating_rating_url",
          value || DEFAULT_CONFIG.rating_url
        );
      },
    });
    ...
    Lampa.SettingsApi.addParam({
      ...
      field: {
        name: "Ключ API",
        description:
          'Зарегистрируйтесь на сайте kinopoiskapiunofficial.tech и введите ваш API-KEY из раздела "Профиль"',
      },
      onChange: function (value) {
    ...
    Lampa.SettingsApi.addParam({
      ...
      onChange: function (value) {
        Lampa.Storage.set(
          "rating_cache_time",
          parseInt(value) || DEFAULT_CONFIG.cache_time
        );
      },
    });
  }
  ...
  function rating_kp_imdb(card) {
    ...
    function getRating() {
      ...
      if (movieRating) {
        ...
        if (config.api_key === "") {
          showError(
            "Не задан API ключ. Рейтинг получить невозможно, зайдите в настройки плагина."
          );
          return;
        }
        searchFilm();
